...
export default function SettingsScreen() {
	...
	const { 
		fetchFeaturedParkInfos,
		lastFeaturedInfoFetch,
		loading: featuredParksLoading,
		error: featuredParksError
	} = useFeaturedParks();
	const {
		fetchFeatureFlags,
	...
	const handleRefreshFeaturedParkInfos = async () => {
		await fetchFeaturedParkInfos();
		// Optionally, show a toast or alert: "Featured parks data refresh initiated."
	};
		...
		>
			...
			<ScrollView ref={scrollViewRef}>
				...
				<View className="p-6 border-t border-charcoal-200 dark:border-charcoal-700 mt-4">
					...
					{/* New button for refreshing featured parks */}
					<TouchableOpacity
						onPress={handleRefreshFeaturedParkInfos}
						disabled={featuredParksLoading}
						className={`flex-row items-center justify-center p-4 rounded-lg shadow-sm mb-3 mt-4 ${featuredParksLoading ? (effectiveTheme === 'dark' ? 'bg-charcoal-600' : 'bg-charcoal-200') : (effectiveTheme === 'dark' ? 'bg-teal-700' : 'bg-teal-600')}`}
					>
						{featuredParksLoading ? (
							<ActivityIndicator size="small" color={effectiveTheme === 'dark' ? getColor('teal-300') : getColor('teal-800')} className="mr-2" />
						) : (
							<Ionicons name="star-outline" size={28} color={effectiveTheme === 'dark' ? 'white' : 'white'} className="mr-2" />
						)}
						<Text className={`ml-3 text-lg font-medium ${effectiveTheme === 'dark' ? 'text-white' : 'text-white'}`}>
							{featuredParksLoading ? 'Refreshing Featured Info...' : 'Force Refresh Featured Info'}
						</Text>
					</TouchableOpacity>
					<Text className="text-sm text-charcoal-600 dark:text-charcoal-400 text-center mt-2">
						{formatLastFetchTime(lastFeaturedInfoFetch)}
					</Text>
					<Text className="text-sm text-charcoal-600 dark:text-charcoal-400 text-center mt-1 mb-2">
						Featured parks info is auto updated daily.
					</Text>
					{featuredParksError && (
						<Text className="text-sm text-red-600 dark:text-red-400 text-center mt-1">
							Error refreshing featured parks: {featuredParksError.message}
						</Text>
					)}
				</View>
				...
				<View className="items-center py-8 mt-4 border-t border-charcoal-200 dark:border-charcoal-700">
					...
					{appVersion && (
						<Text className="text-xs text-charcoal-500 dark:text-charcoal-400 mt-1">
